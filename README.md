...
* It doesn't use STL's string() and map() calls, due to their extensive use of dynamic allocation.
* Gave up the compatibility to Linux and Windows to streamline the code (however, backporting to POSIX compliant platforms should be relatively simple - but then better use the original code, `minihttp`).
